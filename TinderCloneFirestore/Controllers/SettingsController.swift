...
class SettingsController: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        hud.textLabel.text = "Завантажуємо..."
        hud.show(in: view)
    ...
    func createButton(selector: Selector) -> UIButton {
        ...
        button.setTitle("Вибрати фото", for: .normal)
    ...
    fileprivate func setupNavigationItems() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Скасувати", style: .plain, target: self, action: #selector(handleCancel))
        navigationItem.rightBarButtonItems = [
            UIBarButtonItem(title: "Зберегти", style: .plain, target: self, action: #selector(handleSave)),
            UIBarButtonItem(title: "Вихід", style: .plain, target: self, action: #selector(handleLogout))
        ]
    }
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        switch section {
        case 1:
            headerLabel.text = "Ім'я"
        case 2:
            headerLabel.text = "Хоббі"
        case 3:
            headerLabel.text = "Вік"
        case 4:
            headerLabel.text = "Про себе"
        default:
            headerLabel.text = "Вік який шукаєте"
        }
        return headerLabel
    }
    ...
    static let defaultMaxSeekingAge = 70
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 5 {
            ...
            ageRangeCell.minLabel.text = "Мінімум \(minAge)"
            ageRangeCell.minSlider.value = Float(minAge)
            ageRangeCell.maxLabel.text = "Maксимум \(maxAge)"
        ...
        case 1:
            cell.textField.placeholder = "Введіть ім'я"
            cell.textField.text = user?.name
            cell.textField.addTarget(self, action: #selector(handleNameChange), for: .editingChanged)
        case 2:
            cell.textField.placeholder = "Введіть хоббі"
            cell.textField.text = user?.profession
            cell.textField.addTarget(self, action: #selector(handleProfessionChange), for: .editingChanged)
        case 3:
            cell.textField.placeholder = "Введіть вік"
            cell.textField.addTarget(self, action: #selector(handleAgeChange), for: .editingChanged)
            if let age = user?.age {
                cell.textField.text = String(age)
            }
        default:
            cell.textField.placeholder = "Роскажіть про себе"
            cell.textField.text = user?.prosebe
            cell.textField.addTarget(self, action: #selector(handleProsebeChange), for: .editingChanged)
        }
        return cell
    }
    ...
    @objc func handleProsebeChange(textField: UITextField) {
        user?.prosebe = textField.text
    }
    ...
    @objc fileprivate func handleMinAgeChange(slider: UISlider) {
        ...
        ageRangeCell.minLabel.text = "Mінімум \(Int(slider.value))"
        ageRangeCell.maxLabel.text = "Maксимум \(Int(ageRangeCell.maxSlider.value))"
    ...
    @objc fileprivate func handleMaxAgeChange(slider: UISlider) {
        ...
        ageRangeCell.maxLabel.text = "Mінімум \(Int(slider.value))"
        ageRangeCell.minLabel.text = "Mаксимум \(Int(ageRangeCell.minSlider.value))"
    ...
    @objc fileprivate func handleSave() {
        ...
        let hud = JGProgressHUD(style: .dark)
        hud.textLabel.text = "Зберігаємо налаштування"
        hud.show(in: view)
        ...
        Firestore.firestore().collection("users").document(uid).setData(docData) { (error) in
            ...
            if let error = error {
                print("Не вдалося зберегти інформацію:", error)
                hud.textLabel.text = "Помилка під час збереження"
