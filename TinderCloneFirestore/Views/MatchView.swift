...
class MatchView: UIView {
    ...
    var cardUID: String! {
        didSet {
            ...
            query.document(cardUID).getDocument { (snapshot, error) in
                ...
                self.descriptionLabel.text = "Ви та \(user.name ?? "") маєте\nспільні інтереси"
    ...
    fileprivate let descriptionLabel: UILabel = {
        let label = UILabel()
        label.text = "Вітаємо!Ви маєте спільні інтереси"
    ...
    fileprivate let sendMessageButton: SendMessageButton = {
        ...
        button.setTitle("НАПИСАТИ ПОВІДОМЛЕННЯ", for: .normal)
    ...
    fileprivate let keepSwipingButton: KeepSwipingButton = {
        ...
        button.setTitle("Продовжити гортати", for: .normal)
        button.setTitleColor(.white, for: .normal)
        return button
    }()
